...
import dotenv
from flask import jsonify, request  # già importati in app.py, ma per chiarezza
...
import io
from i18n import LANGUAGES, UI_TRANSLATIONS, normalize_language, translate_chat_error, translate_html, translate_server_message, translate_ui
...
@app.before_request
def set_language():
    requested_lang = request.values.get("lang") or request.args.get("lang")
    if requested_lang:
        session["lang"] = normalize_language(requested_lang)
    else:
        session["lang"] = normalize_language(session.get("lang", "en"))


def current_language():
    return normalize_language(session.get("lang", "en"))


def _(text):
    return translate_ui(text, current_language())


def err_msg(message):
    return translate_server_message(message, current_language())
...
def localize_html_response(response):
    if response.content_type and response.content_type.startswith("text/html"):
        response.set_data(translate_html(response.get_data(as_text=True), current_language()))
    return response
...
def format_clifford_tableau_bool(cliff: Clifford) -> str:
    ...
    return f"{_('Shape')}: {arr.shape[0]} x {arr.shape[1]}\n\n" + "\n".join(rows)
...
def generate_routing_analysis(original_text: str, routed_text: str, original_qasm: str | None = None, routed_qasm: str | None = None) -> str:
    if _openai_client is None:
        return err_msg("ChatGPT not configured (missing library or API key).")
    model = os.getenv("OPENAI_MODEL", "gpt-4o-mini")
    language_name = "Italian" if current_language() == "it" else "English"
    user_prompt = (
        ...
        f"Provide a concise summary in {language_name} (10-12 lines), bullet-style, no code. Do NOT use Markdown (no **, no backticks, no headings).\n\n"
    ...
    try:
        ...
        if ("401" in msg) or ("Unauthorized" in msg) or ("api_key" in msg):
            return err_msg("ChatGPT not configured (invalid or missing OPENAI_API_KEY).")
        if ("404" in msg) or (("model" in msg) and ("not found" in msg)):
            return err_msg("ChatGPT analysis unavailable: model not found or not accessible.")
        if ("timeout" in msg) or ("network" in msg):
            return err_msg("ChatGPT analysis unavailable: network error or timeout.")
        return err_msg(f"ChatGPT analysis unavailable: {type(e).__name__}: {e}")
...
def generate_synthesis_comparison_analysis(
    ...
    user_prompt = (
        ...
        f"- Be concise and concrete; cite exact numbers when available. Write the response in {language_name}.\n\n"
        + "\n".join(blocks)
    )
...
def generate_operator_synthesis_analysis(
    original_text: str,
    ...
    user_prompt = (
        "You are a quantum circuit synthesis assistant.\n"
        "Explain what happened after applying connectivity-aware synthesis to this circuit.\n"
        f"Write in {language_name}. Be concise: 6-9 bullet points, plain text, no Markdown.\n"
        "Mention the selected coupling map, routing method, basis gate set, size, depth, CX depth, and SWAP count.\n"
        "Explain what these numbers mean for a student and why routing/translation may change the circuit.\n\n"
        f"Coupling map: {map_type}\n"
        f"Routing method: {routing_method}\n"
        f"Basis gate set: {basis}\n"
        f"Stats: size={stats.get('size')} depth={stats.get('depth')} cx_depth={stats.get('cx_depth')} qubits={stats.get('num_qubits')}\n"
        f"SWAP count: {swap_count if swap_count is not None else 'n/a'}\n\n"
        "[Original circuit - ASCII]\n" + (original_text or "(n/a)") + "\n\n"
        "[Synthesized circuit - ASCII]\n" + (synthesized_text or "(n/a)")
    )
    ...
    try:
        resp = _openai_client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are an expert in Qiskit circuit synthesis, routing, and basis translation."},
                {"role": "user", "content": user_prompt},
            ],
            temperature=0.2,
        )
...
def generate_routing_comparison_analysis(
    ...
    user_prompt = (
        ...
        f"- Be concise and cite exact numbers when available. Write the response in {language_name}.\n\n"
        + "\n".join(blocks)
    )
...
def synthesis_view():
    ...
    op_synth_analysis_text = session.get("op_synth_analysis_text", None)
    op_synth_analysis_hash = session.get("op_synth_analysis_hash", "")
    ...
    if request.method == "POST":
        ...
        def _load_op_qc_or_error():
            ...
            if not op_circuit_json.strip():
                op_error = err_msg("You must build a circuit first.")
                return None
            try:
                return circuit_from_builder_json(op_circuit_json)
            except Exception as e:
                op_error = err_msg(f"Invalid circuit: {type(e).__name__}: {e}")
                return None
        ...
        if action == "op_render_circuit":
            ...
            if file is None or file.filename == "":
                op_error = err_msg("Please select a QASM file to load.")
            else:
                try:
                    qasm_text = file.read().decode("utf-8", errors="replace")
                    qc_loaded = QuantumCircuit.from_qasm_str(qasm_text)
                    ...
                    op_synth_circuit_text_cx = None
                    op_synth_stats_cx = None
                    op_synth_analysis_text = None
                    op_synth_analysis_hash = ""
                    session.pop("op_synth_analysis_text", None)
                    session.pop("op_synth_analysis_hash", None)

                except Exception as e:
                    op_error = err_msg(f"Failed to load QASM: {type(e).__name__}: {e}")
            ...
            if qc is not None:
                ...
                try:
                    ...
                    op_error = err_msg(f"Operator generation failed: {type(e).__name__}: {e}")
            ...
            if qc is not None:
                ...
                try:
                    ...
                    if coupling_map is None and op_selected_routing != "none":
                        op_error = err_msg("Routing requires a coupling map. Please select a coupling map first.")
                    else:
                        layout_list = list(range(qc.num_qubits))
                        ...
                        analysis_hash = _json_hash(
                            f"{op_circuit_json}|{op_selected_map}|{op_selected_routing}|{op_selected_translation_basis}|"
                            f"{op_synth_stats.get('size')}|{op_synth_stats.get('depth')}|{op_synth_stats.get('cx_depth')}|{op_synth_swap_count}|{current_language()}"
                        )
                        if session.get("op_synth_analysis_hash", "") != analysis_hash:
                            op_synth_analysis_text = generate_operator_synthesis_analysis(
                                original_text=op_circuit_text,
                                synthesized_text=op_synth_circuit_text,
                                stats=op_synth_stats,
                                swap_count=op_synth_swap_count,
                                map_type=op_selected_map,
                                routing_method=op_selected_routing,
                                basis=op_selected_translation_basis,
                            )
                            session["op_synth_analysis_text"] = op_synth_analysis_text
                            session["op_synth_analysis_hash"] = analysis_hash
                        else:
                            op_synth_analysis_text = session.get("op_synth_analysis_text", None)

                except Exception as e:
                    op_error = err_msg(f"Synthesis failed: {type(e).__name__}: {e}")
    ...
    return render_template(
        ...
        op_synth_analysis_text=op_synth_analysis_text,
...
def clifford_synthesis_view():
    ...
    def _cliff_ui_options(n: int):
        ...
        if rl_enabled:
            algo_opts = {"clifford_greedy": "Greedy", "clifford_ag": "Aaronson-Gottesman", "RL": "Topology-aware RL synthesizer"}
        else:
            algo_opts = {
                "clifford_greedy": "Greedy",
                "clifford_ag": "AG",
    ...
    def _set_error(msg: str, section: str):
        ...
        cliff_error = err_msg(msg)
        cliff_error_section = section or ""

    def _valid_mode(value: str) -> str:
        value = (value or "").strip()
        return value if value in {"start", "build", "load", "loaded"} else "start"

    if request.method == "GET":
        requested_mode = _valid_mode(request.args.get("mode") or session.get("cliff_mode", "start"))
        if requested_mode == "loaded" and not _has_loaded_operator():
            requested_mode = "start"
        mode = requested_mode
        focus_section = (request.args.get("focus_section") or session.get("cliff_focus_section") or "clifford").strip()

        cliff_circuit_json = session.get("cliff_circuit_json", "")
        cliff_builder_depth = int(session.get("cliff_builder_depth", 10))
        cliff_num_qubits = int(session.get("loaded_cliff_num_qubits") or session.get("cliff_num_qubits", 3))
        cliff_selected_map = session.get("cliff_selected_map", "line")
        cliff_selected_algo = session.get("cliff_selected_algo", "clifford_greedy")

        if mode == "loaded" and _has_loaded_operator():
            try:
                cliff_loaded = Clifford(np.asarray(session.get("loaded_cliff_tableau"), dtype=bool))
                cliff_tableau_bool_text = format_clifford_tableau_bool(cliff_loaded)
                cliff_num_qubits = cliff_loaded.num_qubits
            except Exception:
                mode = "start"
        else:
    ...
    if request.method == "POST":
        ...
        if action in ("cliff_back_to_start", "clear_all"):
            ...
            # Load a Clifford operator from a NumPy tableau or a Clifford QASM circuit.
                ...
                if file is None or file.filename == "":
                    _set_error("Please select a Clifford QASM circuit or a NumPy tableau file (.npy or .npz).", "operator")
                else:
                    try:
                        file_bytes = file.read()
                        filename = (file.filename or "").lower()
                        if filename.endswith(".qasm"):
                            qc_loaded = QuantumCircuit.from_qasm_str(file_bytes.decode("utf-8", errors="replace"))
                            cliff = Clifford(qc_loaded)
                        else:
    ...
    session["cliff_mode"] = mode
    session["cliff_focus_section"] = focus_section
    session["cliff_circuit_json"] = cliff_circuit_json
    session["cliff_builder_depth"] = cliff_builder_depth
    session["cliff_num_qubits"] = cliff_num_qubits
    session["cliff_selected_map"] = cliff_selected_map
    session["cliff_selected_algo"] = cliff_selected_algo
...
@app.route("/clifford_synthesis/save_qasm", methods=["POST"])
def clifford_synthesis_save_qasm():
    circuit_json = request.form.get("cliff_circuit_json", "") or ""
    if not circuit_json.strip():
        return err_msg("You must build a circuit first."), 400

    try:
        qc = circuit_from_builder_json(circuit_json)
        response = make_response(qasm2_dumps(qc))
        response.headers["Content-Type"] = "application/qasm; charset=utf-8"
        response.headers["Content-Disposition"] = 'attachment; filename="clifford_circuit.qasm"'
        return response
    except Exception as e:
        return err_msg(f"Invalid circuit: {type(e).__name__}: {e}"), 400
...
TUTOR_SYSTEM_PROMPT = """You are an expert tutor for Qiskit quantum circuit transpilation, circuit synthesis, and Clifford synthesis.
You help a university student understand how transpilation and synthesis workflows work, not just get answers.

Pedagogical guidelines:
1. Answer directly and clearly. Explain concepts using the student's circuit and data
   as concrete examples. At the end of the explanation, you may add ONE optional question
   to stimulate reflection, but only if it feels natural and useful.
   Do NOT ask questions BEFORE answering.
2. Use the student's current circuit, operator, synthesis settings, and results as concrete examples whenever context is available.
3. Be concise: 3-5 sentences unless the user asks for detail.
4. Respond in the language specified by the runtime instruction.
5. Avoid elaborate Markdown. Short bullet lists are okay. Inline code is okay.
6. Ask for clarification if the question is ambiguous.
7. Say explicitly when the available context is insufficient.
8. Do not invent facts about Qiskit.
"""


def _build_context_block(ctx: dict) -> str:
    if not ctx:
        return "(No context available.)" if current_language() == "en" else "(Nessun contesto disponibile.)"
    ...
    if current_language() == "it":
        section_labels = {
            ...
            "operator_synthesis": "sta lavorando sulla sintesi di un operatore",
            "clifford_build": "sta costruendo un circuito Clifford da sintetizzare",
            "clifford_loaded": "ha caricato un operatore Clifford da sintetizzare",
        }
        lines = []
        sec = ctx.get("section")
        if sec:
        ...
        if ctx.get("synthesis_algorithm"):
            lines.append(f"Algoritmo di sintesi scelto: {ctx['synthesis_algorithm']}.")
        ...
        if ctx.get("synthesized_text"):
            lines.append(f"\nCircuito dopo la sintesi:\n{ctx['synthesized_text']}")
        return "\n".join(lines)

    lines = []
    section_labels = {
        "editor": "is working in the circuit editor",
        "target": "is choosing the coupling map",
        "layout": "is working on the layout stage",
        "routing": "is working on the routing stage",
        "translation": "is working on translation",
        "operator_synthesis": "is working on operator synthesis",
        "clifford_build": "is building a Clifford circuit to synthesize",
        "clifford_loaded": "has loaded a Clifford operator to synthesize",
    }
    sec = ctx.get("section")
    if sec:
        lines.append(f"Current section: the student {section_labels.get(sec, sec)}.")
    if ctx.get("num_qubits"):
        lines.append(f"Number of qubits: {ctx['num_qubits']}.")
    if ctx.get("coupling_map"):
        lines.append(f"Selected coupling map: {ctx['coupling_map']}.")
    if ctx.get("layout_method"):
        lines.append(f"Selected layout method: {ctx['layout_method']}.")
    if ctx.get("routing_method"):
        lines.append(f"Selected routing method: {ctx['routing_method']}.")
    if ctx.get("translation_basis"):
        lines.append(f"Selected translation basis: {ctx['translation_basis']}.")
    if ctx.get("synthesis_algorithm"):
        lines.append(f"Selected synthesis algorithm: {ctx['synthesis_algorithm']}.")
    if ctx.get("circuit_text"):
        lines.append(f"\nStudent's original circuit:\n{ctx['circuit_text']}")
    if ctx.get("layout_pairs"):
        lines.append(f"\nApplied layout (logical -> physical): {ctx['layout_pairs']}")
    if "routed_swap_count" in ctx:
        lines.append(f"\nAfter routing: {ctx['routed_swap_count']} SWAP gates inserted.")
    if "routed_depth" in ctx:
        lines.append(f"Depth after routing: {ctx['routed_depth']}.")
    if ctx.get("routed_text"):
        lines.append(f"\nCircuit after routing:\n{ctx['routed_text']}")
    if ctx.get("synthesized_text"):
...
def api_chat():
    if _gemini_client is None or genai_types is None:
        return jsonify({
            "reply": translate_chat_error("Il tutor non è configurato. Verifica GEMINI_API_KEY nel file .env e che google-genai sia installato.", current_language()),
            "error": True,
        })
    ...
    try:
        ...
        if not isinstance(client_messages, list):
            return jsonify({"reply": translate_chat_error("Formato messaggi non valido.", current_language()), "error": True})

        language_instruction = "Rispondi in italiano." if current_language() == "it" else "Respond in English."
        system_instruction = TUTOR_SYSTEM_PROMPT + "\n\n" + language_instruction + "\n\nCURRENT CONTEXT:\n" + _build_context_block(context)

        gemini_contents = []
        for m in client_messages[-20:]:
        ...
        if not gemini_contents:
            return jsonify({"reply": translate_chat_error("Scrivi una domanda per iniziare.", current_language()), "error": True})
        ...
        for attempt in range(3):
            try:
                ...
                if "503" in str(retry_err) or "UNAVAILABLE" in str(retry_err).upper():
        ...
        if "MAX_TOKENS" in finish_reason.upper():
            return jsonify({
                "reply": translate_chat_error("Risposta troncata per limite di lunghezza. Riprova chiedendo qualcosa di più specifico.", current_language()),
                "error": True,
            })

        if not reply:
            if "SAFETY" in finish_reason.upper():
                return jsonify({"reply": translate_chat_error("La risposta è stata bloccata dai filtri di sicurezza di Gemini. Prova a riformulare la domanda.", current_language()), "error": True})
            return jsonify({"reply": translate_chat_error(f"(Risposta vuota. Motivo: {finish_reason or 'sconosciuto'}.)", current_language()), "error": True})
        ...
        if "api_key" in lower or "api key" in lower or "unauthenticated" in lower or "401" in msg:
            return jsonify({"reply": translate_chat_error("API key Gemini invalida o mancante. Controlla GEMINI_API_KEY nel .env e verifica su https://aistudio.google.com/apikey", current_language()), "error": True})
        if "resource_exhausted" in lower or "429" in msg or "rate" in lower or "quota" in lower:
            return jsonify({"reply": translate_chat_error("Limite di richieste Gemini raggiunto. Il free tier ha 10 richieste/minuto e 250 al giorno. Aspetta un minuto e riprova.", current_language()), "error": True})
        if "not found" in lower or ("model" in lower and "404" in msg):
            return jsonify({"reply": translate_chat_error(f"Modello Gemini non disponibile. Verifica GEMINI_MODEL nel .env. Errore: {e}", current_language()), "error": True})
        return jsonify({"reply": translate_chat_error(f"Errore: {type(e).__name__}: {e}", current_language()), "error": True})
...
def api_translate_chat():
    ...
    try:
        ...
        if not messages:
            return jsonify({"messages": [], "error": False})

        target_lang = normalize_language(data.get("target_lang") or current_language())
        language_name = "Italian" if target_lang == "it" else "English"
        ...
        for i, m in enumerate(messages):
            ...
            translated_messages.append({
                ...
                "content": translate_chat_error(content, target_lang) if m.get("error") else content,
                "error": bool(m.get("error", False)),
            })
            if content and not m.get("error"):
                numbered.append(f"[{i}] {content}")

        # Error messages are translated locally even when Gemini is unavailable.
        if not numbered:
            return jsonify({"messages": translated_messages, "error": False})
        if _gemini_client is None or genai_types is None:
            return jsonify({
                "messages": translated_messages,
                "error": True,
                "detail": translate_chat_error("Chat translation unavailable: Gemini is not configured.", target_lang),
            })

        prompt = (
            f"Translate each of the following chat messages to {language_name}. "
            "Keep quantum computing technical terms unchanged (e.g. SWAP, qubit, CX, coupling map, layout, routing, Clifford, QASM). "
            "Keep each leading [index] marker unchanged. "
            "Return ONLY a valid JSON object with key \"translations\" whose value is an array of strings, "
            "one per input message, in the same order. Do not add any other text.\n\n"
            + "\n".join(numbered)
        )

        models = [
            os.getenv("GEMINI_MODEL", "gemini-2.5-flash"),
            os.getenv("GEMINI_FALLBACK_MODEL", "gemini-2.0-flash-lite"),
        ]
        response = None
        last_error = None
        ...
        non_error_indexes = [
            ...
            if (m.get("content") or "").strip() and not m.get("error")
        ]
        for t_idx, message_index in enumerate(non_error_indexes):
            if t_idx >= len(translations):
                break
            translated = str(translations[t_idx])
            translated = __import__("re").sub(r"^\s*\[\d+\]\s*", "", translated)
            translated_messages[message_index]["content"] = translated
        ...
        if "resource_exhausted" in lower or "429" in lower or "quota" in lower:
            detail = translate_chat_error("Chat translation unavailable: Gemini quota has been reached. Try again later.", target_lang)
        else:
            detail = translate_chat_error(f"Errore: {type(e).__name__}: {e}", target_lang)
        return jsonify({"messages": data.get("messages", []), "error": True, "detail": detail})
